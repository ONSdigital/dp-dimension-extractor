...
func TestUnitExtract(t *testing.T) {
	...
	Convey("test successful extraction of dimensions", t, func() {
		Convey("where all dimensions are unique", func() {
			...
			So(err, ShouldBeNil)
			So(dimensions["123_Time"], ShouldResemble, dimension.Request{DimensionID: "time", Value: "2016/17", Code: "Year", Label: "2016/17", CodeList: "1234-435435-5675"})
			So(dimensions["123_League"], ShouldResemble, dimension.Request{DimensionID: "league", Value: "PL01", Code: "PL01", Label: "Premier-League", CodeList: "dgdfg-435435-5675"})
		})
		...
		Convey("where some dimensions are unique", func() {
			...
			dimensions, err := extract.Extract()
			So(err, ShouldBeNil)
			So(dimensions["123_League"], ShouldResemble, dimension.Request{DimensionID: "league", Value: "Championship", Label: "Championship", CodeList: "dgdfg-435435-5675"})
		})
		...
		Convey("where no dimensions are unique", func() {
			...
			dimensions, err := extract.Extract()
			So(err, ShouldBeNil)
			So(dimensions["123_League"], ShouldResemble, dimension.Request{DimensionID: "", Value: ""})
		})
	})
